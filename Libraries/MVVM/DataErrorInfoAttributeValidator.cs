...
namespace MVVM
{
    ...
    public partial class DataErrorInfoAttributeValidator
    {
        ...
        private partial class PropertyValidator
        {
            ...
            private static bool StringArraysEqual(string[] str1, string[] str2)
            {
            ...
            private static string FromCamelCase(string value) =>
             MyRegex().Replace(value, @" ${a}");
            [GeneratedRegex("(?<a>(?<!^)((?:[A-Z][a-z])|(?:(?<!^[A-Z]+)[A-Z0-9]+(?:(?=[A-Z][a-z])|$))|(?:[0-9]+)))")]
